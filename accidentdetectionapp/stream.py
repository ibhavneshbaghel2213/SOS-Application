...
def detect(boxes,t1):
    ...
    return False
def send_message():
    client = vonage.Client(key="9bf5e1cd", secret="9RNpPwM6DoLJ2fHn")
    sms = vonage.Sms(client)
    responseData = sms.send_message(
        {
            "from": "Bhavnesh",
            "to": "917902178705",
            "text": "Urgent \n Accident happened at raj mandi. Please send ambulance as soon as possible. Google map link :- https://www.google.com/maps/search/?api=1&query=47.5%2C-122.3316393 ",
        }
    )
...
def sendmail():
    ...
    gmail_list.append("dazzlingbhavnesh@gmail.com")
    print("gmail list", gmail_list)
    email_subject = "Urgent please send ambulance."
    email_body = "Accident happened please send ambulance as soon as possible. Google map link :- https://www.google.com/maps/search/?api=1&query=47.5%2C122.3316393 "
    return print(send_mail(email_subject, email_body, settings.EMAIL_HOST_USER, gmail_list, fail_silently=False))
...
class streaming(object):
    def __init__(self):
        ...
        # self.video_capture = cv2.VideoCapture("C:\\Users\\lenovo\\accident3.mp4")
        self.model1 = torch.hub.load('ultralytics/yolov5', 'custom', path='C:\\Users\\lenovo\\My Project\\SOS Application\\best (2).pt',device='cpu')
        self.model = torch.hub.load('ultralytics/yolov5', 'custom', path='C:\\Users\\lenovo\\My Project\\SOS Application\\accident2.pt',device='cpu')
